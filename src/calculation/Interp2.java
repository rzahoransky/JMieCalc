...
/*                  factorization and forward substitution */
      for(i=2; i<=n; i++){
        sub[i] = sub[i]/diag[i-1];           // find multiplier
        diag[i] = diag[i] - sub[i]*sup[i-1]; // update upper triang. matrix
        b[i] = b[i] - sub[i]*b[i-1];         // update rhs vector
      }
      b[n] = b[n]/diag[n];                   // solve upper triang. system by back subst.
      for(i=n-1;i>=1;i--){
